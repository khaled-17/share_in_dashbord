...
export const Settings: React.FC = () => {
  ...
  const handleProjectSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading(
      projectEditing ? "جاري التحديث..." : "جاري الإضافة...",
    );
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading(
      expenseEditing ? "جاري التحديث..." : "جاري الإضافة...",
    );
    try {
  ...
  const handleExpenseEdit = (t: ExpenseType) => {
    ...
    setExpenseFormData({
      exptype_id: t.exptype_id,
      exptype_name: t.exptype_name,
      category: t.category || "",
    });
  ...
  const handleRevenueEdit = (t: RevenueType) => {
    ...
    setRevenueFormData({
      revtype_id: t.revtype_id,
      revtype_name: t.revtype_name,
      ...
      render: (type: ProjectType) => (
        <div className="flex gap-2">
          <Button
            variant="secondary"
            size="sm"
            onClick={() => handleProjectEdit(type)}
          >
            تعديل
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => handleProjectDelete(type.id)}
          >
            حذف
          </Button>
        </div>
      ...
      render: (t: ExpenseType) => (
        <div className="flex gap-2">
          ...
          <Button
            variant="danger"
            size="sm"
            onClick={() => handleExpenseDelete(t.id)}
          >
            حذف
          </Button>
        </div>
      ...
      <Card title="الإعدادات">
        <div className="flex gap-4 mb-6 border-b pb-4">
          <button
            ...
            className={`pb-2 px-4 ${activeTab === "expense" ? "border-b-2 border-primary-600 text-primary-600 font-bold" : ""}`}
          >
            أنواع المصروفات
          </button>
          <button
            onClick={() => setActiveTab("revenue")}
            className={`pb-2 px-4 ${activeTab === "revenue" ? "border-b-2 border-primary-600 text-primary-600 font-bold" : ""}`}
          >
            أنواع الإيرادات
          </button>
          <button
            onClick={() => setActiveTab("project")}
            className={`pb-2 px-4 ${activeTab === "project" ? "border-b-2 border-primary-600 text-primary-600 font-bold" : ""}`}
          >
            أنواع المشاريع
          </button>
          <button
            onClick={() => setActiveTab("company")}
            className={`pb-2 px-4 ${activeTab === "company" ? "border-b-2 border-primary-600 text-primary-600 font-bold" : ""}`}
          >
            ملف الشركة
          </button>
          <button
            onClick={() => setActiveTab("reviews")}
            className={`pb-2 px-4 ${activeTab === "reviews" ? "border-b-2 border-primary-600 text-primary-600 font-bold" : ""}`}
          >
            آراء العملاء
          </button>
        </div>
            ...
            <Button
              onClick={() => {
                if (activeTab === "expense")
                  setExpenseShowForm(!expenseShowForm);
                else if (activeTab === "revenue")
                  setRevenueShowForm(!revenueShowForm);
                else if (activeTab === "reviews")
                  setReviewShowForm(!reviewShowForm);
                else {
                  if (!projectShowForm) {
                    setProjectShowForm(true);
                    setProjectFormData({
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    وصف مختصر للنشاط (يظهر في الهيدر) *
                  </label>
                  <textarea
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    عن الشركة (يظهر في الموقف المالي)
                  </label>
                  <textarea
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 text-right"
                    rows={4}
          ...
          <>
            {projectShowForm && (
              ...
              >
                ...
                <Input
                  label="اسم النوع *"
                  value={projectFormData.type_name}
                  onChange={(e) =>
                    setProjectFormData({
                      ...projectFormData,
            ...
            {projectLoading ? (
              ...
              <Table columns={projectColumns} data={projectTypes} />
            )}
          </>
        )}
        ...
        {activeTab === "expense" && (
          <>
            ...
            {expenseLoading ? (
              ...
              <Table columns={expenseColumns} data={expenseTypes} />
            )}
          </>
        )}
        ...
        {activeTab === "revenue" && (
          <>
            ...
            {revenueLoading ? (
              ...
              <Table columns={revenueColumns} data={revenueTypes} />
            )}
          </>
        )}
              ...
              >
                ...
                <div className="flex gap-2 mt-6">
                  ...
                  <Button
                    type="button"
                    variant="secondary"
                    onClick={() => setReviewShowForm(false)}
                  >
                    إلغاء
                  </Button>
                </div>
