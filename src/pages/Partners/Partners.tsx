  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMsg.includes("بالفعل")) {
        toast.error("عذراً، كود الشريك هذا تم استخدامه مسبقاً", {
          id: loadingToast,
          duration: 5000,
        });
      } else {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (p: Partner) => (
        <span
          className={
            p.current_capital >= p.initial_capital
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            مشاهدة وإدارة جميع بيانات الشركاء ورأس المال
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label="كود الشريك"
              value={
                isEditing ? formData.partner_code : "سيتم التوليد تلقائياً"
              }
              disabled
            ...
            {!isEditing && (
              <div className="md:col-span-2">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  يمكن زيادة رأس المال لاحقاً عن طريق سندات القبض
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <Button type="submit">
              {isEditing ? "تحديث البيانات" : "إضافة الشريك"}
            </Button>
          </div>
        </form>
      </Modal>
