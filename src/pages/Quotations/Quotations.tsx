...
export const Quotations: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [quotationsData, customersData, projectTypesData] =
        await Promise.all([
  ...
  const validateForm = () => {
    ...
    if (!formData.project_type_id)
      errors.project_type_id = "يرجى اختيار نوع المشروع";
    if (!formData.project_name.trim())
      errors.project_name = "اسم المشروع مطلوب";
  ...
  const validateItem = () => {
    ...
    if (itemFormData.unit_price <= 0)
      errors.unit_price = "يجب أن يكون السعر أكبر من صفر";
  ...
  const saveItem = () => {
    ...
    const newItem = {
      ...itemFormData,
      total: itemFormData.unit_price * itemFormData.quantity,
    };
  ...
  const columns = [
    ...
    { key: "project_type_name", header: "النوع" },
    {
      key: "totalAmount",
      header: "الإجمالي",
      render: (row: QuotationRow) => `${row.totalAmount.toLocaleString()} ج.م`,
    },
  ...
  const itemColumns = [
    { key: "description", header: "الوصف" },
    {
      key: "unit_price",
      header: "سعر الوحدة",
      render: (it: Item) => `${it.unit_price.toLocaleString()} ج.م`,
    },
    { key: "quantity", header: "الكمية" },
    {
      ...
      header: "الإجمالي",
      render: (it: Item) =>
        `${(it.unit_price * it.quantity).toLocaleString()} ج.م`,
    },
    {
      key: "actions",
      header: "الإجراءات",
  ...
  const projectTypeOptions = [
    ...
    ...projectTypes.map((p) => ({ value: p.type_id, label: p.type_name })),
  ];
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <Input
                ...
                value={
                  isEditing && currentId
                    ? `QUO-${String(currentId).padStart(4, "0")}`
              ...
              <Select
                ...
                onChange={(e) => {
                  setFormData({ ...formData, project_type_id: e.target.value });
                  setFormErrors({ ...formErrors, project_type_id: "" });
                }}
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setFormErrors({ ...formErrors, project_name: "" });
                }}
            ...
            <div className="space-y-4 mb-6">
              <div className="flex justify-between items-center bg-white p-4 rounded-lg border">
                <h4 className="font-bold text-lg text-primary-900 text-right">
                  بنود العرض
                </h4>
                <Button
                  type="button"
            ...
            <div className="flex justify-between items-center p-6 bg-primary-600 text-white rounded-xl shadow-lg">
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-black">
                  {parseFloat(formData.totalamount).toLocaleString()} ج.م
                </span>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) => {
                ...
                setItemErrors({ ...itemErrors, unit_price: "" });
              }}
          ...
          <div className="p-4 bg-gray-50 rounded-lg flex justify-between items-center border border-dashed">
            ...
            <span className="text-2xl font-black text-primary-700">
              {(
                itemFormData.unit_price * itemFormData.quantity
              ).toLocaleString()}{" "}
              ج.م
            </span>
          </div>
