...
export const Checks: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (c: any) => (
        <span
          className={`px-2 py-1 rounded-full text-xs ${c.receipt_voucher_id ? "bg-blue-100 text-blue-800" : "bg-orange-100 text-orange-800"}`}
        >
    ...
    },
    {
      ...
      render: (c: any) => (
        <span
          className={`px-2 py-1 rounded-full text-xs ${getStatusColor(c.status)}`}
        >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            متابعة جميع الشيكات الواردة والصادرة وحالاتها
          </p>
        </div>
      </div>
          ...
          {currentCheck && (
            <div className="bg-gray-50 p-4 rounded-lg text-sm space-y-2">
              <p>
                ...
                {currentCheck.check_number}
              </p>
              <p>
                <span className="font-semibold">المستفيد:</span>{" "}
                {currentCheck.beneficiary_name}
              </p>
              <p>
                <span className="font-semibold">المبلغ:</span>{" "}
                {formatCurrency(currentCheck.amount)}
              </p>
            </div>
          )}
