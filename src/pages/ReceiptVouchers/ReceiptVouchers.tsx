...
export const ReceiptVouchers: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.received_from.trim())
      newErrors.received_from = "اسم المستلم من مطلوب";
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "هل أنت متأكد من حذف هذا السند؟ سيتم عكس تأثيره على رأس المال إن وجد.",
      )
    )
      return;
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, received_from: e.target.value })
              }
